...
## Independent provenance contract workflow

The bounded-model-submission provenance capability is generated and checked by
committed, Windows-long-path-aware Node tooling:

```text
npm run materialize:provenance-contract
npm run verify:provenance-contract
npm run regenerate:provenance-contract
npm run inspect:provenance-contract
```

`verify:provenance-contract` checks schema meta-validity, negative schema
controls, catalog and artifact-type registration, semantic hashes, and all
sixteen four-body lineages. `regenerate:provenance-contract` starts from an
empty temporary root, materializes the authority spine, adopts and finalizes
the AST authorities, projects signed bodies, verifies them, and requires every
generated byte to match the repository.

Use the inspection command instead of PowerShell's legacy file APIs for paths
longer than 260 characters. It lists every capability file by default, or
